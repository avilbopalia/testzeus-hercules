<#
.SYNOPSIS
    Pull the smartaitester/smart-ai-tester Docker image, ensure .env and agents_llm_config.json
    exist, optionally launch Chrome or Firefox in remote debugging mode (based on
    user selection), and run Hercules in Docker.

    Usage example:
    .\run_smart_ai_tester_docker.ps1 -BrowserChoice Chrome
    .\run_smart_ai_tester_docker.ps1 -BrowserChoice Firefox
    .\run_smart_ai_tester_docker.ps1 -BrowserChoice None
#>
...
###############################################################################
# 1) Pull the SmartAITester Docker image
###############################################################################
Write-Host "Pulling the latest smartaitester/smart-ai-tester Docker image..."
...
if (-not (Test-Path ".env")) {
    ...
    if (Test-Path "env-example") {
        ...
        Invoke-WebRequest -Uri "https://raw.githubusercontent.com/test-zeus-ai/smart-ai-tester/main/env-example" -OutFile "env-example" -ErrorAction SilentlyContinue
...
if (-not (Test-Path "agents_llm_config.json")) {
    ...
    if (Test-Path "agents_llm_config-example.json") {
        ...
        Invoke-WebRequest -Uri "https://raw.githubusercontent.com/test-zeus-ai/smart-ai-tester/main/agents_llm_config-example.json" -OutFile "agents_llm_config-example.json" -ErrorAction SilentlyContinue
...
###############################################################################
Write-Host "Running smartaitester/smart-ai-tester:latest Docker container..."
docker run --env-file=.env `
  -v "${PWD}\agents_llm_config.json:/smart-ai-tester/agents_llm_config.json" `
  -v "${PWD}\opt:/smart-ai-tester/opt" `
  --rm -it smartaitester/smart-ai-tester:latest

Write-Host "SmartAITester run completed."

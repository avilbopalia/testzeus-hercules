...
from smart_ai_tester.core.agents_llm_config_manager import AgentsLLMConfigManager
from smart_ai_tester.core.playwright_manager import PlaywrightManager
from smart_ai_tester.core.smart_ai_tester_core import SmartAITester
from smart_ai_tester.utils.cli_helper import async_input  # type: ignore
...
class BaseRunner:
    ...
    def __init__(
        ...
        self.smart_ai_tester_core: SmartAITester | None = None
    ...
    async def initialize(self) -> None:
        ...
        self.smart_ai_tester_core = await SmartAITester.create(
            self.stake_id,
    ...
    async def clean_up(self) -> None:
        """Clean up resources."""
        if self.smart_ai_tester_core:
            await self.smart_ai_tester_core.shutdown()
        if self.browser_manager:
            await self.browser_manager.stop_playwright()

    def get_agents_map(self) -> Dict[str, Any]:
        """Get the agents map from SmartAITester."""
        if not self.smart_ai_tester_core or not self.smart_ai_tester_core.agents_map:
            return {}
        agents_map = cast(Dict[str, Any], self.smart_ai_tester_core.agents_map)
        return agents_map
    ...
    async def process_command(self, command: str) -> tuple[Any, float]:
        ...
        if command:
            ...
            if self.smart_ai_tester_core:
                ...
                result = await self.smart_ai_tester_core.process_command(command, current_url)
